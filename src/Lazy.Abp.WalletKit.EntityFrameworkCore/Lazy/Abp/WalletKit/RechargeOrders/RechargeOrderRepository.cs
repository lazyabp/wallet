...
namespace Lazy.Abp.WalletKit.RechargeOrders
{
    public class RechargeOrderRepository : EfCoreRepository<IWalletKitDbContext, RechargeOrder, Guid>, IRechargeOrderRepository
    {
        ...
        public override async Task<IQueryable<RechargeOrder>> WithDetailsAsync()
        {
        ...
        {
            var query = await GetListQuery(userId, gateway, status, isClosed, minAmount, maxAmount, creationAfter, creationBefore, filter);
